...
def create_ball():
    ...
    mid_cup = ball.get_rect(center = (750, pos))
    return mid_cup
...
def create_gold_ball():
    pos = random.choice(gold_ball_height)
    mid_ufo = ball.get_rect(center = (1000, pos))
    return mid_ufo
    
def move_gold_ball(ufos):
    for c in ufos:
        c.centerx -= speedX
    return ufos
 
def draw_gold_ball(ufos):
    for c in ufos:
        screen.blit(gold_ball, c)

def check_collision_gold_ball(ufos):
    for c in ufos:
        if bird_rect.colliderect(c):
            ufos.remove(c)
            return False
...
def create_bullet():
    ...
    mid_ufo = ball.get_rect(center = (1200, pos))
    return mid_ufo
...
gold_ball = pygame.image.load('assets/gold-ball.png').convert_alpha()
ball_list = []
gold_ball_list = []
    #tao dan  
...
gold_ball_height = [250, 280, 300, 320]
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == spawncup:
            cup_list.append(create_ball())
            gold_ball_list.append(create_gold_ball())
    ...
    if game_active:
        collision = check_collision_ball(ball_list)
        collision1 = check_collision_gold_ball(gold_ball_list)
        ...
        if collision2 == False: 
            ...
            if count > 40: count = 0
        # ball
        cup_list = move_ball(ball_list)
        draw_ball(cup_list) 
        # gold_ball
        ufo_list = move_gold_ball(gold_ball_list)
        draw_gold_ball(gold_ball_list) 
        #bullet
